...
function draw() {
  ...
  for (let babiesShown = babies.length - 1; babiesShown >= 0; babiesShown--) {
    babies[babiesShown].move();
    babies[babiesShown].show();
    babies[babiesShown].update();
    
    for (let milksLeft = 0; milksLeft < milks.length; milksLeft++){
      if (isTouching(babies[babiesShown], milks[milksLeft])){
        milks.splice(milksLeft, 1);
      }
    }
    
    let babyIsDead = babies[babiesShown].lifespan == 0;
    
    if (babyIsDead) {
      babies.splice(babiesShown, 1);
      break;
    }
  }
  ...
  for (let milksShown = 0; milksShown < milks.length; milksShown++) {
    ...
    if (framesDelayed > spawnDelay) {
      for (milksChecked = 0; milksChecked < milks.length; milksChecked++) {
        ...
        if (isDifferentMilk && milkDistance < spriteWidth) {
          x = random(width);
          y = random(height - 100);
          size = 0.8;
